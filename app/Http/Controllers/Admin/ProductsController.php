    ...
    public function postAdd(AddProRequest $request)
    {
        ...
        if ($request->hasFile('img')) {
            ...
            foreach ($files as $key => $value) {
                ...
                $destinationPath = public_path('images/app/thumbnails');
                $img = Image::make($value->getRealPath());
                $img->resize(265, 150, function ($constraint) {
    ...
    public function postEdit($id, Request $request)
    {
        ...
        if ($request->hasFile('imgedit')) {
            ...
            foreach ($object as $item) {
                ...
                $filename = public_path("images/app/thumbnails/$item");
                File::delete($filename);
            }
        ...
        if ($result == 1) {
            return back()->with('<script>window.close</script>');
        } else {
            return redirect()->route('shoes.products.index')->with('error', 'Có lỗi xảy ra !');
        }
    }
