...
@section('content')
    <div class="cart">
        @if( Cart::count() != 0 )
            ...
            <div class="col-sm-5 cart-content-right">
                ...
                <script>
                    var check = {!! json_encode(optional(auth()->user())->only('id', 'email')) !!}
                    function checkLogin() {
                        if(check == null){
                            Swal.fire({
  title: 'Oh, có vấn đề?',
  ...
  if (result.isConfirmed) {
    $('#login').modal('show');
                ...
                <input type="button" value="Tiến hành thanh toán" style="width: 100%" class="button btn btn-primary" onclick="checkLogin();"/>
            </div>
        </div>
        @else
